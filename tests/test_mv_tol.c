// The tolerance-gated Metal decode matvec (suite plan arc B, "a second,
// tolerance-gated Metal matvec"; the conclusion of
// docs/negative-result-metal-multirow-matvec.md).
//
// The shipped decode matvec is the CPU<->GPU byte-identity contract: each
// output row accumulates over `for (i = tiisg; i < n; i += 32)` into one
// scalar and finishes with one simd_sum, which pins which lane owns which
// block. The 2026-08-12 sweep measured that route at its ceiling — the only
// levers it admits are load width (neutral) and row assignment (zero-sum) —
// and concluded that closing the roofline gap needs the REASSOCIATING
// transformations, on a second kernel promoted by teacher-forced tolerance
// the way the tiled prefill GEMM already is.
//
// This is that gate. The fast route factors the q4_0 zero-point out of the
// inner loop (sum(q-8)*y*d becomes d*sum(q*y) - 8d*sum(y)) and accumulates in
// float4 lanes, so it is NOT bit-identical by construction and a gate
// demanding identity would be a gate that gets disabled.
//
// This follows test_i8_tol.c, with the differences that come from where the
// route lives:
//
//   * it is GPU-only and DECODE-only: the fast kernel is selected at
//     n_col == 1, so every configuration's PREFILL is bit-identical and only
//     the teacher-forced decode positions can move. That is exactly how the
//     route reaches production.
//   * the negative control is a re-run of the reference rather than a
//     thread-count change. It pins a property the rest of this file depends
//     on: Metal decode is run-to-run deterministic, so a difference the gate
//     reports is attributable to the kernel and not to the GPU.
//   * the top-1 bar is ZERO flips. The promotion bar for this lever is 0/64,
//     matching what the TC prefill path had to clear before it was defaulted
//     on. A combo that cannot hold it is not promoted; it is not a combo
//     whose bar gets widened.
//
//     ./test-mv-tol models/e2b-q40.gguf
//     ./test-mv-tol models/granite-4.1-8b-Q4_0.gguf 8   # force 8 GPU layers
//
// Default model is test.gguf (F32 toy): the harness runs and self-skips,
// because F32 has no fast kernel.
#include "runner.h"
...
#define MV_DEV_FRAC  0.005    // mean|dlogit| bound, fraction of mean range
...
typedef struct {
    const char *name;
    int         mv;          // gpu_mv_force argument while this config runs
    bool        available;
...
int main(int argc, char **argv) {
    ...
    bool has_mv_type = false;
    for (uint64_t i = 0; i < gf.n_tensors; i++)
        if (gf.tensors[i].type == T_Q4_0 || gf.tensors[i].type == T_Q8_0)
            has_mv_type = true;
    ...
    printf("mv-tol: %s | %d tokens, %d teacher-forced positions\n",
           path, n_tok, STEPS);
    if (!has_mv_type) {
    ...
    unsigned long before = gpu_mv_dispatches();
    for (int i = 0; i < N_CFG; i++)
        run_config(&cfgs[i], path, toks, n_tok, n_vocab);
    unsigned long fired = gpu_mv_dispatches() - before;
    gpu_mv_force(-1);
    ...
    printf("  mv-fast   vs scalar : %lu fast matvecs, mean|dlogit| %.6f = "
           "%.5f of mean logit range %.2f (limit %.3f)\n",
           fired, impl, frac_of_range, range, MV_DEV_FRAC);
    printf("  mv-fast   vs scalar : top1 diff %d/%d (limit %d), worst flip "
           "margin %.4f of range\n", n_diff, STEPS, DISAGREE_MAX, worst);

    ck(frac_of_range <= MV_DEV_FRAC,
