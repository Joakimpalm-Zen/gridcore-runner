...
SRC = src/gguf.c src/compat.c src/quants.c src/tokenizer.c src/model.c src/sample.c \
      src/vramreg.c \
      src/template.c src/jsonmode.c src/schema.c src/quantize.c src/engine.c src/json.c src/http.c src/server.c \
      src/main.c $(GPU_SRC)
...
TEST_RESP_SM_SRC = tests/test_responses_sm.c src/gguf.c src/compat.c \
                  src/quants.c src/tokenizer.c src/model.c src/sample.c \
                  src/jsonmode.c src/schema.c src/json.c src/engine.c \
                  src/template.c src/vramreg.c src/http.c $(GPU_SRC)
...
test: $(TEST_JSON_SCHEMA) $(TEST_JSON_OOM) $(TEST_SCHEMA_OOM) $(TEST_SAMPLER) \
	...
	@if $(PYTHON) -c "import pytest" >/dev/null 2>&1; then \
		...
		$(PYTHON) -m pytest -q tests/test_ornith_cpu.py tests/test_ornith_reference.py tests/test_compat_matrix.py tests/test_arch_admission.py tests/test_cli_files.py tests/test_bench_json.py tests/test_mtp_admission.py tests/test_compare_llamacpp.py tests/test_release_check.py tests/test_eseries.py tests/test_stress_models.py; \
